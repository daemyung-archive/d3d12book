...
void BlurFilter::OnResize(UINT newWidth, UINT newHeight)
{
    if ((mWidth != newWidth) || (mHeight != newHeight))
    {
        ...
        // 새로운 리소스들이 생성되었기 때문에, 디스크립터들을 업데이트 해야합니다.
        BuildDescriptors();
    }
}
                         ...
                         int blurCount)
{
    ...
    for (int i = 0; i < blurCount; ++i)
    {
        ...
        // 한 그룹이 256 픽셀을 처리할 때, 이미지를 처리하기 위해선 몇개의 그룹이 필요할까요?
...
{
    // 압축 텍스쳐는 UAV로 사용할 수 없습니다.
    // 압축 텍스쳐를 쓰시면 아래와 같은 에러 메세지가 출력됩니다.
